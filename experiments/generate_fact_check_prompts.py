...
def create_openai_message(prompt, custom_id=None):
    msg = {"body": {"messages": [{"role": "user", "content": prompt}]}}
    if custom_id:
...
def generate_prompts(claims_file, metadata_file, output_prompts, output_metadata):
    ...
    for _, row in tqdm(claims_df.iterrows(), total=len(claims_df)):
        ...
        custom_id = f"claim_{count}"
        prompts.append(create_openai_message(prompt_text, custom_id))
...
def process_results(results_file, metadata_file, output_csv):
    ...
    id_to_meta = {r['custom_id']: r for r in meta_records}
...
if __name__ == "__main__":
    ...
    gen_parser.add_argument("--output-prompts", default="../fact_check_prompts.jsonl")
    gen_parser.add_argument("--output-metadata", default="../fact_check_metadata.jsonl")

    proc_parser = subparsers.add_parser("process")
    proc_parser.add_argument("--results-file", required=True)
    proc_parser.add_argument("--metadata-file", required=True, help="Path to the metadata jsonl file generated by the generate command")
    proc_parser.add_argument("--output-csv", default="../results/fact_check_results.csv")
