...
PROMPT_TEMPLATE = """You are a legal expert. Read a short claim about a supreme court case. Decide whether the claim is factually consistent with the details of the case.
...
def generate_prompts(claims_file, metadata_file, output_prompts, output_metadata):
    ...
    # Create a lookup for metadata to ensure we filter same cases
    meta_lookup = set()
    for _, row in meta_df.iterrows():
        if pd.notna(row['name']):
            meta_lookup.add(row['name'])
...
def process_results(results_file, claims_file, output_csv):
    ...
    for res in tqdm(results):
        ...
        # Pattern: Claim: ...\n\n## Output Format:
        match = re.search(r"Claim:\s*(.*?)\s*## Output Format:", prompt, re.DOTALL)
        if not match:
            continue
...
if __name__ == "__main__":
    ...
    gen_parser.add_argument("--output-prompts", default="../factuality_naive_prompts.jsonl")
    gen_parser.add_argument("--output-metadata", default="../factuality_naive_metadata.jsonl")

    proc_parser = subparsers.add_parser("process")
    proc_parser.add_argument("--results-file", default="../factuality_naive_output_vllm.jsonl")
    proc_parser.add_argument("--claims-file", default="../claims_raw.csv")
    proc_parser.add_argument("--output-csv", default="../results/factuality_naive_results.csv")
