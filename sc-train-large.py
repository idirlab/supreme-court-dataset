...
class SemanticSimilarityTrainer:
    def __init__(self, csv_file='sc-claims_v4.csv', model_name='Qwen/Qwen3-Embedding-0.6B'):
        """
    ...
    def prepare_data(self):
        """Prepare the training data and split into train/test"""
        ...
        self.test_df = self.df[split_idx:]
        
        print(f"Train set: {len(self.train_df)} cases")
        print(f"Test set: {len(self.test_df)} cases")
        
        # Create mapping from case name to index for full dataset (needed for evaluation)
        self.name_to_idx = {name: idx for idx, name in enumerate(self.df['name'].values)}
        self.case_names = self.df['name'].values
        
    def create_training_examples(self, claim_columns=['claim_1', 'claim_2', 'claim_3', 'claim_4', 'claim_5']):
        """
        Create training examples using multiple claim columns from the training set
        
        Args:
            claim_columns: List of claim columns to use for training
        """
        print(f"Creating training examples using {claim_columns} from training set...")
        ...
        for claim_column in claim_columns:
            ...
            for idx, row in self.train_df.iterrows():
                ...
                other_names = [name for name in self.train_df['name'].values if name != case_name]
    ...
    def train_model(self, claim_columns=['claim_1', 'claim_2', 'claim_3', 'claim_4', 'claim_5'], epochs=4, batch_size=8):
        """
        Train the semantic similarity model using the training set
        ...
        # Create training examples from training set
    ...
    def evaluate_model(self, test_claim_columns=['claim_1', 'claim_2', 'claim_3', 'claim_4', 'claim_5'], model_name="Trained Model", save_predictions=True):
        """
        Evaluate the model using recall@1, recall@5, and recall@10 on the test set
        ...
        # Encode all case names (full dataset for retrieval)
        ...
        for claim_col in test_claim_columns:
            ...
            for idx, row in tqdm(self.test_df.iterrows(), total=len(self.test_df), desc=f"{model_name} - {claim_col}"):
        ...
        if total_queries > 0:
            ...
            overall_recall_1 = overall_recall_5 = overall_recall_10 = 0.0
    ...
    def evaluate_both_models(self, test_claim_columns=['claim_1', 'claim_2', 'claim_3', 'claim_4', 'claim_5']):
        """
        Evaluate both the naive and trained models for comparison on test set
...
def main():
    ...
    if trainer.model_exists(model_path):
        ...
        results = trainer.evaluate_both_models(test_claim_columns=['claim_1', 'claim_2', 'claim_3', 'claim_4', 'claim_5'])
        
    else:
        print("No trained model found. Starting training...")
        
        # Train the model using all claims from training set
        trainer.train_model(claim_columns=['claim_1', 'claim_2', 'claim_3', 'claim_4', 'claim_5'], epochs=1, batch_size=8)
